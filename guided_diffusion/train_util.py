...
class TrainLoop:
    ...
    def run_loop(self):
        while (
            ...
            # print(th.max(batch),th.min(batch))
            
            #"default","norm_max_pixel","min_max"
            batch = batch.view(batch.shape[0]*batch.shape[1],batch.shape[-3],batch.shape[-2], batch.shape[-1])

            if self.args.data_processing == "min_max":

                batch = (batch - th.min(batch))/(th.max(batch)-th.min(batch)+1e-12)

            elif self.args.data_processing == "norm_max_pixel":
                ...
                batch = (batch + abs(min_values)+1e-12) / max_values
                # batch = (batch - th.min(batch))/(th.max(batch)-th.min(batch)+1e-12)

            # print(batch.shape)


            # Compute the maximum value for each image in the batch
            ...
            # print([b[i] for i in range(len(b)) if i %1024 == 0])
            # print(len(b))
            ...
            # assert False
            # print("next passed")
            # batch = th.Tensor(batch, device="cuda")
            # print("convert passed")

            self.run_step(batch, cond)
            # print("run step passed")
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], 1024*self.microbatch):
            # print(i)
            ...
            # print("training_losses: ", self.diffusion.training_losses)
            # print("micro: ", micro)
            # print("t: ", t)
            ...
            if (loss < self.lowest_loss) and ((self.step - 1) >= 1):
                self.save(best=True)
    ...
    def save(self, best=False):
        ...
        if best:
            ...
            with bf.BlobFile(bf.join(get_blob_logdir(), filename), "wb") as f:
                th.save(state_dict, f)
            print(f"New Best: {self.lowest_loss}")
        else:
