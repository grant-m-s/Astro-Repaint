...
class GaussianDiffusion:
    ...
    def _undo(self, img_out, t):
        # memory_before = th.cuda.memory_allocated("cuda")

        beta = _extract_into_tensor(self.betas, t, img_out.shape)
        # memory_after = th.cuda.memory_allocated("cuda")
        # latent_size = memory_after - memory_before
        # print("beta after: ",latent_size/(1024**3))
        # assert False
    ...
    def p_mean_variance(
        ...
        # print("start of p_mean_var:", memory_before)
        ...
        # print("timesteps:",self._scale_timesteps(t))
        model_output = model(x, self._scale_timesteps(t), **model_kwargs)

        # free, total = th.cuda.mem_get_info(0)
        # memory_after = (total - free) / 1024 ** 3
        # print("model output:", memory_after-memory_before)
        # assert False

        assert model_output.shape == (B, C * 2, *x.shape[2:])
        model_output, model_var_values = th.split(model_output, C, dim=1)
        # if t[0].item() == 1:
        #     print(th.max(model_output),th.min(model_output))
            # assert False
        # free, total = th.cuda.mem_get_info(0)
        # memory_before = (total - free) / 1024 ** 3
        # print("before self.model_mean_type == ModelVarType.LEARNED:", memory_before)
        ...
        # free, total = th.cuda.mem_get_info(0)
        # memory_before = (total - free) / 1024 ** 3
        # print("before self.model_mean_type == PREVIOUS_X:", memory_before)
        ...
        # print(self.model_mean_type)
            # if t[0].item() == 1:
            #     print(th.max(pred_xstart),th.min(pred_xstart))
        ...
        # free, total = th.cuda.mem_get_info(0)
        # memory_before = (total - free) / 1024 ** 3
        # print("before return:", memory_before)
        # if t[0].item() == 1:
        #     print(th.max(model_mean),th.min(model_mean))
    ...
    def p_sample(
        ...
        # free, total = th.cuda.mem_get_info(0)
        # memory_before = (total - free) / 1024 ** 3
        # print("start of p_sample:", memory_before)
        # free, total = th.cuda.mem_get_info(0)
        # memory_after = (total - free) / 1024 ** 3
        # latent_size = memory_after - memory_before
        ...
        if conf.inpa_inj_sched_prev:
            ...
            # print("inpa_inj_sched_prev - True")

            if pred_xstart is not None:
                # print("pred_xstart is not None")
                ...
                # print(gt.shape)
                # import matplotlib.pyplot as plt
                # plt.imshow(gt.cpu().numpy()[0][0])
                # plt.show()

                # plt.imshow(gt.cpu().numpy()[1][0])
                # plt.show()
                # assert False
                ...
                if conf.inpa_inj_sched_prev_cumnoise:
                    # print("inpa_inj_sched_prev_cumnoise is true")
                    weighed_gt = self.get_gt_noised(gt, int(t[0].item()))
                else:
                    # print("inpa_inj_sched_prev_cumnoise is false")
        ...
        # print("out shape: ",out["mean"].shape)
        ...
        result = {"sample": sample,
                  "pred_xstart": out["pred_xstart"], 'gt': model_kwargs.get('gt'), 'ids':model_kwargs.get('y')}
    ...
    def p_sample_loop_progressive(
        ...
        if conf.schedule_jump_params:
            ...
            # latent_size = memory_after - memory_before
            # print("schedule_times: ",latent_size/(1024**3))
...
# 743,542,563,628
            ...
            for t_last, t_cur in time_pairs:
                ...
                # print("t_last_t: ",t_last_t.shape)
                # print(th.cuda.memory_allocated("cuda")/(1024**3))

                # print("t_last: ", t_last, "t_cur",t_cur)
                ...
                if t_cur < t_last:  # reverse
                    with th.no_grad():
                        ...
                        # print("image before shape: ",image_before_step.shape)
                        # free, total = th.cuda.mem_get_info(0)
                        # memory_after = (total - free) / 1024 ** 3
                        # latent_size = memory_after - memory_before
                        # print("image befroe step: ",latent_size/(1024**3), memory_after)
                        ...
                        # memory_after = (total - free) / 1024 ** 3
                        # latent_size = memory_after - memory_before
                        # print("after out: ",latent_size/(1024**3), memory_after)
                        # assert False

                        image_after_step = out["sample"]
                        # print(image_after_step.shape)

                            # plt.imshow
                        pred_xstart = out["pred_xstart"]
                        # if t_cur == 1:
                        #     for j in range(30):
                        #         mosaic = "ABCDEFGHIJKLMNOPQRSTU"
                        #         r_ints = np.random.randint(0,high=1024,size=(len(mosaic)))
                        #         r_ints[0] = 323
                        #         fig, axes = plt.subplot_mosaic(mosaic,figsize=(len(mosaic)*10,10))
                        #         for i in range(len(mosaic)):
                        #             axes[mosaic[i]].imshow(pred_xstart[r_ints[i],0].cpu(), cmap="Greys_r")
                        #             axes[mosaic[i]].set_title(r_ints[i])
                        ...
                        # if t_cur % 5 == 0:
                        if t_cur not in list(shown.keys()):
                            shown[t_cur] = 0
                        else:
                            shown[t_cur] += 1
                    ...
                    # memory_after = th.cuda.memory_allocated(device)
                    # latent_size = memory_after - memory_before
                    # print("after .clone: ",latent_size/(1024**3))
                    # memory_before = th.cuda.memory_allocated(device)
                    ...
                    # memory_after = th.cuda.memory_allocated(device)
                    # latent_size = memory_after - memory_before
                    # print("self.undo: ",latent_size/(1024**3))
                    pred_xstart = out["pred_xstart"]
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    # free, total = th.cuda.mem_ge.t_info(0)
    # memory_after = (total - free) / 1024 ** 3
    # latent_size = memory_after - memory_before
    # print("res: ",latent_size/(1024**3), memory_after)
    # assert False
    # memory_before = th.cuda.memory_allocated("cuda")
    ...
    # memory_after = th.cuda.memory_allocated("cuda")
    # latent_size = memory_after - memory_before
    # print("res after: ",latent_size/(1024**3))
    # assert False
